...
namespace PuzzleImageGenerator.Mega.Painter
{
    class MegaImageProp : ImageProp
	{
            ...
            : base(configs)
		{
            ...
            /**
            var cornerOuterLength = Math.Sin(Math.PI * 54 / 180) * (LongFaceDist - CenterDist) / Math.Sin(LARGEANGLE);
            var smallAngle = Math.Sqrt(Math.Pow(cornerOuterLength, 2) + Math.Pow(LongFaceDist, 2) - (2 * cornerOuterLength * LongFaceDist * Math.Cos(Math.PI * 54 / 180)));
            **/
